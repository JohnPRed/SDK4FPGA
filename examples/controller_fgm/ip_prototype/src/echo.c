...
void print_app_header()
{
	xil_printf("\n\r");
	xil_printf("\n\r");
	xil_printf("------------------- FPGA Interface framework   --------------------\n\r");
	xil_printf("----------- by Andrea Suardi [a.suardi@imperial.ac.uk] -------------\n\r");
	xil_printf("---------------------- Rev 1.0 April 2014 -------------------------\n\r");
	xil_printf("\n\r");
	xil_printf("\n\r");
	if (MY_TCP_UDP==1)
			...
			if (packet_type==1) //reset IP
						{
					...
					else if (packet_type==2) //start IP
							{
								...
								XFoo_SetByte_x_out_offset(&xcore,x_OUT_DEFINED_MEM_ADDRESS);
					...
					else if (packet_type==4) //read data from DDR
					{
						...
						switch (packet_internal_ID)
						{
							...
							if (DEBUG)
								printf("read x_out\n\r");
							memcpy(outvec,x_out_ptr_ddr+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset,(ETH_PACKET_LENGTH_RECV-2)*4);
							break;
