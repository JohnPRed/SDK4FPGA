				...
				uint32_t* status)
{
	...
	#pragma HLS INTERFACE ap_none register     port=byte_x_out_offset
	#pragma HLS RESOURCE core=AXI4LiteS    variable=byte_x_out_offset metadata="-bus_bundle BUS_A"

	data_t_interface  x_in[N];
	data_t_interface  num_iter_in[N];
	data_t_interface  x_out[N];
	...
	*status=0; //IP running
	...
	memcpy(num_iter_in,(const data_t_memory*)(memory_inout+byte_num_iter_in_offset/4),N*sizeof(data_t_memory));
	...
	input_cast_loop:for (int i=0; i< N; i++)
	{
		...
		num_iter_in_int[i]=(data_t)num_iter_in[i];
	}
	...
	//num_iter_in_int[N] -> data type is data_t
	//Output vectors are:
	//x_out_int[N] -> data type is data_t
	...
	{
		x_out[i]=(data_t_interface)x_out_int[i];
	}

	//write results vector y_out to DDR
	memcpy((data_t_memory *)(memory_inout+byte_x_out_offset/4),x_out,N*sizeof(data_t_memory));
