			...
			if (packet_type==1) //reset IP
						{
					...
					else if (packet_type==2) //start IP
							{
								...
								XFoo_SetByte_x0_in_offset(&xcore,x0_IN_DEFINED_MEM_ADDRESS);
								XFoo_SetByte_x_ref_in_offset(&xcore,x_ref_IN_DEFINED_MEM_ADDRESS);
								XFoo_SetByte_u_out_offset(&xcore,u_OUT_DEFINED_MEM_ADDRESS);
					...
					else if (packet_type==3) //write data DDR
					{
						...
						switch (packet_internal_ID)
						{
							...
							if (DEBUG)
								printf("write x0_in\n\r");
							memcpy(x0_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,sizeof(inputvec));
							break;
							
							case 1: //x_ref_in
							if (DEBUG)
								printf("write x_ref_in\n\r");
							memcpy(x_ref_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,sizeof(inputvec));
							break;
					...
					else if (packet_type==4) //read data from DDR
					{
						...
						switch (packet_internal_ID)
						{
							...
							if (DEBUG)
								printf("read u_out\n\r");
							memcpy(outvec,u_out_ptr_ddr+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset,(ETH_PACKET_LENGTH_RECV-2)*4);
							break;
