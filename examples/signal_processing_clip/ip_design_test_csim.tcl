...
for {set i 0} {$i < $num_simulation} {incr i} {
	...
	set status [ catch { exec $MatlabPath --nospash -nodesktop -r write_stimulus($i,$sim_type,$sim_offset,$num_iter)} output ]
	...
	set status [ catch { exec $MatlabPath --nospash -nodesktop -r read_results($i,$sim_type,$sim_offset,$num_iter)} output ]
